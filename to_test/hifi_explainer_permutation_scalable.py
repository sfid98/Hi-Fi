...
class HiFiExplainerScalable:
    ...
    def _predict_with_permutation(self, X_source: np.ndarray, mapped_active_indices: list) -> np.ndarray:
        """
        Esegue una predizione usando il modello pre-addestrato.
        Le feature non in active_indices vengono rese non informative tramite permutazione.
        """

        if not mapped_active_indices:
            return np.full_like(self.y_train, np.mean(self.y_train))
        # Mappa gli indici originali (es. 7) ai nuovi indici di X_train (es. 6)

        # Inizia con una copia pulita delle features di training
        X_permuted = X_source.copy()
        num_features = X_source.shape[1]

        # Itera su tutte le colonne delle features
        for i in range(num_features):
            # Se una colonna NON è nella lista di quelle attive, la permutiamo
            if i not in mapped_active_indices:
                X_permuted[:, i] = np.random.permutation(X_permuted[:, i])

        # Esegui la predizione sulla matrice con le colonne "rotte"
        return self.trained_model.predict(X_permuted)

    def _loco_calculator_permutation(self, X_source: np.ndarray, driver_idx: int, context_indices: list) -> float:
        """
        Calcola il LOCO usando predizioni su dati permutati.
        """
        ...
        # Errore del modello ridotto (solo il contesto è attivo, il driver è permutato)
        preds_reduced = self._predict_with_permutation(X_source,mapped_context_indices)
        error_reduced = np.mean((self.y_train - preds_reduced) ** 2)

        # Errore del modello completo (sia il driver che il contesto sono attivi)
        preds_full = self._predict_with_permutation(X_source, [mapped_driver_idx] + mapped_context_indices)
    ...
    def _calculate_hoi(self, driver_idx: int, available_indices: list, is_red: bool):
        ...
        initial_loco = self._loco_calculator_permutation(self.X_train, driver_idx, context_indices=[])
        ...
        while len(remaining_indices) > 0 and should_continue:
            ...
            for candidate_idx in remaining_indices:
                ...
                loco_val = self._loco_calculator_permutation(self.X_train, driver_idx, current_context_indices)
            ...
            if self.n_surrogates > 0:
                ...
                for _ in range(self.n_surrogates):
                    ...
                    loco_surr = self._loco_calculator_permutation(X_surrogate, driver_idx,
    ...
    def standard_loco(self):
        ...
        predictions_full = self._predict_with_permutation(self.X_train, all_feature_indices)
        ...
        for i in range(num_features):
            ...
            predictions_reduced = self._predict_with_permutation(self.X_train, active_indices_reduced)
