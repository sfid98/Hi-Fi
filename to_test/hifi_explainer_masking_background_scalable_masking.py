...
class HiFiExplainerScalable:
    ...
    def fit(self, data: np.ndarray, target_idx: int, masking_rate=0.3):
        """
        Addestra il modello su una versione "corrotta" del dataset per renderlo
        robusto al masking, senza usare partial_fit.
        """
        print(f"Avvio dell'addestramento robusto con masking_rate={masking_rate}...")
        ...
        # Memorizza le medie che serviranno sia per corrompere i dati che per l'analisi
        ...
        # --- FASE DI CORRUZIONE DEL DATASET ---
        print("Creazione del dataset di addestramento corrotto...")
        X_corrupted = self.X_train.copy()
        n_samples, n_features = self.X_train.shape

        # Per ogni riga, scegli un sottoinsieme casuale di feature da mascherare
        for i in range(n_samples):
            # Scegli casualmente gli indici delle colonne da mascherare
            num_to_mask = np.random.randint(1, n_features)

            cols_to_mask = np.random.choice(n_features, num_to_mask, replace=False)

            # Sostituisci i valori di quelle colonne con la loro media
            X_corrupted[i, cols_to_mask] = self.feature_means[cols_to_mask]
        # --- FINE FASE DI CORRUZIONE ---

        # Addestra il modello una singola volta sul dataset corrotto
        self.trained_model = self.model_factory()
        self.trained_model.fit(X_corrupted, self.y_train)

        print("Modello robusto addestrato.")
